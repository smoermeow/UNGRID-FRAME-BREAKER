...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    answer: "UnGrid uses Gemini 3 Pro, which requires a personal paid API key. You can either select your own key via the official selection dialog or manually paste your key into the authentication settings. This ensures you only use your own credits."
  },
  {
...
const FAQSection = () => {
  ...
  return (
    <div className="w-full border-t border-[#2B2B2B] pt-8 mt-12">
...
const AuthOverlay = ({ 
  ...
  const [manualKey, setManualKey] = useState(localStorage.getItem('ungrid_api_key') || '');

  const saveKey = () => {
    localStorage.setItem('ungrid_api_key', manualKey);
    onAuthorized();
  };
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/95 backdrop-blur-xl p-6">
      <div className="w-full max-w-xl bg-black border-2 border-[#FFD43B] p-10 shadow-[0_0_100px_rgba(255,212,59,0.1)] text-center relative overflow-hidden">
        <button onClick={onClose} className="absolute top-4 right-4 text-[#FFD43B] hover:text-white"><X size={24}/></button>
        ...
        <p className="text-[#E0E083] text-sm font-mono mb-8 leading-relaxed">
          UnGrid utilizes <span className="text-white font-bold">Gemini 3 Pro</span>. 
          To protect project credits, you must provide your own API key.
        </p>

        <div className="space-y-4 mb-8">
           <div className="text-left">
              <label className="text-[10px] text-[#5CFF72] font-bold uppercase block mb-1 tracking-widest">Manual API Key</label>
              <input 
                type="password" 
                value={manualKey} 
                onChange={(e) => setManualKey(e.target.value)}
                placeholder="Paste key here (saved locally)..." 
                className="w-full bg-[#111] border border-[#2B2B2B] text-white p-3 font-mono text-xs focus:outline-none focus:border-[#FFD43B]"
              />
              <button 
                onClick={saveKey}
                className="w-full mt-2 bg-[#FFD43B] text-black font-bold py-2 hover:bg-white transition-all uppercase text-xs"
              >
                Save Local Key
              </button>
           </div>
           ...
           <div className="relative py-4">
              <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-[#2B2B2B]"></div></div>
              <div className="relative flex justify-center text-[10px] uppercase font-bold"><span className="bg-black px-2 text-[#555]">OR</span></div>
           </div>

           <button 
              onClick={handlePlatformAuth}
              className="w-full border-2 border-[#5CFF72] text-[#5CFF72] font-black py-3 hover:bg-[#5CFF72] hover:text-black transition-all uppercase text-xs flex items-center justify-center gap-2"
           >
              <Zap size={14} fill="currentColor"/> Connect via Google AI Studio
           </button>
        </div>

        <div className="mt-8 pt-6 border-t border-[#2B2B2B] flex flex-col gap-3">
           <a href="https://aistudio.google.com/app/apikey" target="_blank" className="text-[#E0E083] text-xs font-mono underline hover:text-[#5CFF72]">Get a Free API Key</a>
        </div>
      </div>
    </div>
  );
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (gridLayout === '1x3') {
      setAspectRatio('9:16');
    } else if (gridLayout === '1x4') {
      setAspectRatio('21:9');
    } else if (gridLayout === '2x4') {
      setAspectRatio('1:1');
    }
  }, [gridLayout]);
  ...
  const handleImageSelected = async (file: File) => {
    try {
      ...
      else if (detectedRatio === '9:16') setGridLayout('1x4');
      setPanels([]); 
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-black p-4 md:p-8 relative selection:bg-[#5CFF72] selection:text-black">
      ...
      {isAuthOpen && <AuthOverlay onClose={() => setIsAuthOpen(false)} onAuthorized={() => setIsAuthOpen(false)} />}
      ...
      {isParamsOpen && appMode === 'grid_split' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
           <div className="bg-black border-2 border-[#5CFF72] p-6 w-[450px] shadow-[0_0_20px_rgba(92,255,114,0.2)] animate-in fade-in zoom-in duration-200">
              ...
              <div className="space-y-6">
                   <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                       <button onClick={() => setGridLayout('3x3')} className={`p-2 border text-[10px] font-bold uppercase transition-all flex flex-col items-center justify-center gap-1 ${gridLayout === '3x3' ? 'bg-[#5CFF72] text-black border-[#5CFF72]' : 'border-[#2B2B2B] text-[#E0E083] hover:border-[#E0E083]'}`}>
                        ...
                        <button onClick={() => setGridLayout('1x4')} className={`p-2 border text-[10px] font-bold uppercase transition-all flex flex-col items-center justify-center gap-1 ${gridLayout === '1x4' ? 'bg-[#5CFF72] text-black border-[#5CFF72]' : 'border-[#2B2B2B] text-[#E0E083] hover:border-[#E0E083]'}`}>
                           <Ratio size={14} /> 1x4 (ULTRA)
                        </button>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                       <button onClick={() => setAspectRatio('21:9')} className={`p-2 border text-[10px] font-bold uppercase transition-all ${aspectRatio === '21:9' ? 'bg-[#5CFF72] text-black border-[#5CFF72]' : 'border-[#2B2B2B] text-[#E0E083] hover:border-[#E0E083]'}`}>21:9</button>
      ...
      <header className="max-w-7xl mx-auto mb-8 border-b-2 border-[#2B2B2B] pb-6 flex flex-col items-center justify-center gap-6 relative">
         <div className="absolute right-0 top-0">
          <button onClick={() => setIsAuthOpen(true)} className="flex items-center gap-2 text-[#2B2B2B] hover:text-[#FFD43B] transition-colors" title="Settings / API Key">
             <Key size={16} />
             <span className="text-xs font-bold uppercase hidden md:inline">AUTH SETUP</span>
          </button>
        </div>
      ...
      <main className="max-w-7xl mx-auto">
                    ...
                    <div className="space-y-8 flex-grow">
                      <div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                          ...
                          <button onClick={() => setGridLayout('1x4')} className={`p-4 border text-left transition-all ${gridLayout === '1x4' ? 'border-[#5CFF72] bg-[#5CFF72]/10' : 'border-[#2B2B2B] hover:border-[#E0E083]'}`}>
                             <div className="flex items-center gap-2 mb-2 text-[#FFD43B] font-bold uppercase text-sm"><Ratio size={16} /> 1x4 (ULTRA)</div>
                          </button>
                          <button onClick={() => setGridLayout('2x4')} className={`p-4 border text-left transition-all ${gridLayout === '2x4' ? 'border-[#5CFF72] bg-[#5CFF72]/10' : 'border-[#2B2B2B] hover:border-[#E0E083]'}`}>
                             <div className="flex items-center gap-2 mb-2 text-[#5CFF72] font-bold uppercase text-sm"><Layers size={16} /> 2x4 (GRID)</div>
                          </button>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                         ...
                         <div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <button onClick={() => setAspectRatio('21:9')} className={`flex-1 py-3 border text-[10px] font-bold uppercase transition-all ${aspectRatio === '21:9' ? 'bg-[#5CFF72] text-black border-[#5CFF72]' : 'border-[#2B2B2B] text-[#E0E083] hover:border-[#E0E083]'}`}>21:9</button>
                ...
                {panels.map((panel) => (
                  <div key={panel.id} className="relative group bg-[#050505] border border-[#2B2B2B] hover:border-[#5CFF72] transition-colors p-2">
                    ...
                    <div 
                      ...
                      className={`relative ${aspectRatio === '9:16' ? 'aspect-[9/16]' : aspectRatio === '21:9' ? 'aspect-[21/9]' : 'aspect-video'} bg-black overflow-hidden mb-2 cursor-pointer`}
                    >
                  ...
                  <div className="bg-[#050505] border border-[#2B2B2B] p-4">
                      ...
                      {chainSource ? (
                          ...
                          <ImageDropzone 
                            ...
                            compact={true} 
                            title="SOURCE" 
                            description={<p>[INPUT REQ]: TARGET_FRAME_FOR_CLEANING<br/>[TYPE]: SINGLE_PANEL</p>} 
                          />
                      )}
                  </div>
                  <div className="bg-[#050505] border border-[#2B2B2B] p-4">
                      ...
                      {chainRef ? (
                          ...
                          <ImageDropzone 
                            ...
                            compact={true} 
                            title="REF" 
                            description={<p>[INPUT REQ]: CHARACTER_STYLE_REFERENCE<br/>[SOURCE]: FIDELITY_ANCHOR</p>} 
                          />
                      )}
                  </div>
               </div>
        ...
        {/* --- FAQ SECTION VISIBLE ON ALL MODES --- */}
        ...
        <div className="fixed bottom-0 left-0 w-full bg-black/90 backdrop-blur border-t border-[#2B2B2B] py-2 px-4 flex justify-between items-center text-[10px] font-mono text-[#5CFF72] z-[150]">
           ...
           <div className="flex gap-4 text-[#2B2B2B]">
             <span>BYOK_ENABLED // UNGRID.OS</span>
           </div>
        </div>
