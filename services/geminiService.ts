  ...
  const generate = async () => {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: [
        {
          parts: [
            { inlineData: { mimeType: 'image/png', data: targetData } }, // Image 1 (Target)
            { inlineData: { mimeType: 'image/png', data: referenceData } }, // Image 2 (Reference)
            { text: prompt }
          ]
        }
      ],
...
export const runChainCleaningStep = async (
  ...
  const generate = async () => {
    ...
    // 1. Target Image (Always first)
    parts.push({ inlineData: { mimeType: 'image/png', data: currentData } });

    // 2. Reference Image (Optional, second)
    ...
    // Simplified prompt to avoid 500 errors. No complex system instructions or Markdown headers.
    let prompt = "";
